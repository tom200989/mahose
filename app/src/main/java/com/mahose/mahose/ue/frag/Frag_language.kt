...
class Frag_language : RootFrag() {
    ...
    private fun initAdapter() {
        ...
        adapter.onSwitchLanguageListener = this::transferLanguage // 点击切换语言
    ...
    private fun transferLanguage(lang_coun: String) {
        ...
        SPUtils.get(activity).putString(HoseCons.CURRENT_LANG, language + "-" + if (TextUtils.isEmpty(country)) "default" else country)
        activity.recreate()
    }
    ...
    private fun initLanguage(): ArrayList<LanguageBean> {
        val language_country = SPUtils.get(activity).getString(HoseCons.CURRENT_LANG, "")
        Logma.i(TAG, "lang = $language_country")
        val languages: ArrayList<LanguageBean> = ArrayList()
        for (lang_coun in HoseCons.COUNTRYS) {
    ...
    private fun getLanguageName(lang_coun: String): String {
        ...
        if (split.size > 1) {
            val language = split[0]
            val country = split[1]
            val locale: Locale = if (TextUtils.isEmpty(country) or (country == "default")) {
                Locale(language)
            } else {
