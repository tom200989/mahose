...
class Frag_splash : RootFrag() {
    ...
    override fun onNexts(p0: Any?, p1: View?, p2: String?) {
        ...
        Handler(Looper.getMainLooper()).postDelayed({ checkUpgrade() }, 200)
    }
    ...
    private fun checkUpgrade() {
        Logma.vsd(TAG, "checkUpgrade() -> 进入检查升级流程")
        ...
        updateHelper.onCheckEndListener = { isneed ->
            // 如需升级 - 直接弹出下载视图
            if (isneed) {
                // TODO: 2021/10/11  弹出下载视图 - 强制升级

            } else {// 不需要升级的 - 直接跳转到主页
                Logma.vsd(TAG, "checkUpgrade() -> 跳转到主页")
